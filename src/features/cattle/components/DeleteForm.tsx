...
export default function DeleteForm({
  ...
  const { deleteCattle, isDeleting } = useCattle();
  ...
  const handleDelete = async () => {
    deleteCattle(id, {
      onSuccess: () => {
        toast.custom((t) => (
          <CustomToast
            t={t}
            title="Exito"
            message="El animal ha sido eliminado exitosamente."
            success={true}
          />
        ));
        onClose();
      ...
      onError: (error) => {
        ...
        const message =
          axiosError.response?.data?.message ?? "Error al eliminar el animal";
    ...
    <div className="relative bg-white w-full max-w-md rounded-lg shadow-lg overflow-hidden flex flex-col">
      ...
      <div className="p-6 space-y-6">
        ...
        <p className="text-sm text-text-primary">
          ¿Estás seguro de que deseas eliminar el animal{" "}
          <span className="font-bold text-primary">{cattleName}</span>? Esta
