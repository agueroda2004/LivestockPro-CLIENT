...
import type { DashboardCattleByLand } from "../dashboard";
...
type Props = {
  data: DashboardCattleByLand[];
  isLoading: boolean;
};

export default function CattleByLandBarChart({ data, isLoading }: Props) {
  if (isLoading) {
    return (
  ...
  if (data.length === 0) {
    return (
      <ChartSkeleton
        height={320}
        message="Aún no hay ganado asignado a fincas"
      />
    );
  }
