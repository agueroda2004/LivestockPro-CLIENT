...
export default function DeleteForm({ landName, id, onClose }: DeleteFormProps) {
  const { deleteLand, isDeleting } = useLand();
  ...
  const handleDelete = () => {
    deleteLand(id, {
      onSuccess: () => {
        toast.custom((t) => (
          <CustomToast
            t={t}
            title="Exito"
            message="La finca ha sido eliminada exitosamente."
            success={true}
          />
        ));
        onClose();
      ...
      onError: (error) => {
        ...
        const message =
          axiosError.response?.data?.message ?? "Error al eliminar la finca";
    ...
    <div className="relative bg-white w-full max-w-md rounded-lg shadow-lg overflow-hidden flex flex-col">
      ...
      <div className="p-6 space-y-6">
        ...
        <p className="text-sm text-text-primary">
          ¿Estás seguro de que deseas eliminar la finca{" "}
          <span className="font-bold text-primary">{landName}</span>? Esta
