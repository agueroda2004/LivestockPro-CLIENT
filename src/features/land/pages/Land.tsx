...
export default function Land() {
  const { getLands, isLoadingLands } = useLand();
  ...
  return (
    <main className="flex-1 overflow-y-auto custom-scrollbar p-container-padding-desktop">
      <PageHeader
        ...
        Description="Administra y monitorea el estado de tus propiedades rurales en tiempo real."
        buttonText="Crear"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-2 p-1">
        ...
        getLands && getLands.data.length > 0 ? (
          getLands.data.map((land) => (
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow flex flex-col overflow-hidden group">
              <div className="p-6">
                {/* Begin: Land Information */}
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg text-primary mb-1">{land.name}</h3>
                    <span
                      className={`items-center px-2 py-1 rounded-full text-[10px]  text-black uppercase tracking-wider font-bold ${land.active ? "bg-green-500/20" : "bg-gray-300"}`}
                    >
                      {land.active ? "Activa" : "Inactiva"}
                    </span>
                    <span
                    ...
                    <div className="w-55 flex items-center gap-1 mt-2">
                      <span
                      ...
                      <p className=" text-text-primary text-sm text-nowrap overflow-hidden text-ellipsis">
                        {land.ubication}
                      </p>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow flex flex-col overflow-hidden group">
            <div className="p-6 flex flex-col items-center justify-center">
              ...
              <p className="text-sm text-gray-400 mt-2 text-center">
                Comienza creando tu primera finca para administrar tus
                propiedades rurales de manera eficiente.
              </p>
            </div>
          </div>
        )}
      ...
      {forms.create || forms.edit || forms.delete ? (
        <Overlay>
          ...
          {forms.edit && selectedLand && (
            <Form
              editedLand={selectedLand}
